...
#MRMOVL + RMMOVL
#IMRMOVL writes the register file in memory stage by forwarding. So in original hcl, there 
#exists a load/use hazard. But given the special situation: RMMOVL only needs the register
#information in memory stage so we can avoid this hazard by forwaring the register 
#information from MRMOVL to RMMOVL when MRMOVL is in the memory stage and 
#RMMOVL is in the execute stage.
#ANDL + JLE
#In the original hcl, there exists a mispredicted branch hazard in this situation. In order to 
#avoid this hazard, f_pc will be D_valP if E_src is equal to E_srcB and E_valA > 0 when JLE is 
#in decode stage and ANDL is in execute stage. But if some other actions aren't taken, 
#hcl will still judge a mispredicted hazard, so d_dstE will be set to 8 when this situation 
#happens. The 8 set here will be use to make sure that stall and bubble won't be set to 1
...
int f_pc = [
             D_icode == IJXX && E_icode == IOPL && E_srcA == E_srcB && E_valA > 0 : D_valP;
	# Mispredicted branch.  Fetch at incremented PC
	M_icode == IJXX && !M_Cnd && M_dstE !=8 : M_valA;
...
int d_dstE = [
             D_icode == IJXX && E_icode == IOPL && E_srcA == E_srcB && E_valA > 0 : 8;
...
int e_valA = [
             M_icode in {IMRMOVL, IPOPL, ILEAVE } && E_icode in {IRMMOVL, IPUSHL} && E_srcA == M_dstM : m_valM;
             1: E_valA;
];
...
bool F_stall =
	...
	!(E_icode in { IMRMOVL, IPOPL, ILEAVE } &&D_icode in {IRMMOVL, IPUSHL}  && d_srcA == E_dstM ) &&
...
bool D_bubble =
	# Mispredicted branch
	(E_icode == IJXX && !e_Cnd) && E_dstE !=8 ||
	# Stalling at fetch while ret passes through pipeline
	# but not condition for a load/use hazard
	!(E_icode in { IMRMOVL, IPOPL, ILEAVE} && !(E_icode in { IMRMOVL, IPOPL, ILEAVE } &&D_icode in {IRMMOVL, IPUSHL}  && d_srcA == E_dstM) && E_dstM in { d_srcA, d_srcB }) &&
	  IRET in { D_icode, E_icode, M_icode };
