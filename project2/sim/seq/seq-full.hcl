...
#The instructions compute an immediate and a value which is stored in a register and #then store the result in the same given register. The notation icode:ifun indicates the #two components of the instruction byte, while rA:rB indicates the two components #of the register specifier byte. The notation M1[x] indicates accessing (either reading #or writing) 1 byte at the memory location x,while M4[x] indicates accessing 4 bytes.
#                         iaddl V, rB
